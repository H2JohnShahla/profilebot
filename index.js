...
const developers = '618454945183825940';

client.on('message', wolf => {
	if (wolf.content === prefix + 'help') {
		////هنا  الامر
		var embed = new Discord.RichEmbed()
			.setColor('RANDOM') ////لون الامبد
			.setDescription(`
...
client.on('message', message => {
	...
	if (message.content.startsWith('$خلفيه')) {
		...
		if (dataPro[message.author.id].walls[args[0]]) {
			...
			dataPro[message.author.id].wallSrc =
				dataPro[message.author.id].walls[args[0]].src;
			message.channel.send(
				`**${message.author.username}**|  تم تغير الخلفية بنجاح`
			);
		} else {
			message.channel.send(
				`**${message.author.username}**|  انت لا تملك هذه الخلفية`
			);
		}
	}
});
...
client.on('message', message => {
	...
	if (message.content === '$خلفيات') {
		let embed = new Discord.RichEmbed()
			.setDescription(
				`**لشراء الخلفية استخدم امر $شراء و رقم الخلفية , لوضع الخلفية استخدم امر $خلفية**`
			)
			...
			.setImage(
				'https://cdn.discordapp.com/attachments/444184423056015370/445203129470812171/wWu5HdBj.jpg'
			);
...
client.on('message', message => {
	...
	if (message.content.startsWith('$buy')) {
		...
		if (!args[0]) {
			message.channel.send(
				`** لشراء الخلفية استخدم امر $شراء و رقم الخلفية , لوضع الخلفية استخدم امر $خلفلية ورقم الخلفية**`
			);
		} else if (dataPro[message.author.id].walls == wallpapers[args[0]]) {
			message.reply('انت تملك هذه الخلفيه مسبقاً');
		} else if (wallpapers[args[0]].price > games[sender.id].credits) {
			message.channel.send(
				`**${sender.username}**| انت لا تملك المال الكافي لشراء هذه الخلفية`
			);
		} else if (wallpapers[args[0]].price < games[sender.id].credits) {
			games[sender.id].credits =
				games[sender.id].credits - wallpapers[args[0]].price;
			dataPro[message.author.id].ai = true;
			dataPro[message.author.id].walls[args[0]] = wallpapers[args[0]];
			message.channel.send(
				`**${
					message.author.username
				}**|  تم شراء الخلفية بنجاح استخدم امرخلفية ${args[0]}لاستخدامها`
			);
		}
	}
});
	...
	if (message.content.startsWith(prefix + 'rep')) {
		...
		if (profile[message.author.id].reps != moment().format('L')) {
			...
			message.channel.send(
				`** :up:  |  ${
					message.author.username
				} has given ${getvalueof} a reputation point!**`
			);
		} else {
			message.channel.send(
				`**:stopwatch: |  ${
...
client.on('message', message => {
	...
	if (message.content.startsWith(prefix + '$rep')) {
		...
		{
			profile[message.author.id].reps = profile[
				message.author.id
			].reps = new Date().getTime();
			profile[ment.id].rep += 1;
			message.channel
				.send(
					`** :up:  |  ${
						message.author.username
					} has given ${ment} a reputation point!**`
				)
				.then(
					() => (profile[message.author.id].lastDaily = new Date().getTime())
				);
		}
		if (
			profile[message.author.id].reps &&
			new Date().getTime() - message.mentions.users.first() <
				60 * 1000 * 60 * 24
		) {
			...
			return message.channel.send(
				`:stopwatch: |  ${
					message.author.username
				}, you can award more reputation in ${pretty(r, { verbose: true })}`
			);
		}
	}
});
	...
	if (message.content.startsWith(prefix + 'daily')) {
		if (games[sender.id].lastDaily != moment().format('day')) {
			...
			message.channel.send(
				`**${
					message.author.username
				} you collect your \`200\` :dollar: daily pounds**`
			);
		} else {
			message.channel.send(
				`**:stopwatch: | ${
					message.author.username
				}, your daily :yen: credits refreshes ${moment()
					.endOf('day')
					.fromNow()}**`
			);
		}
...
client.on('message', message => {
	...
	if (message.content === '$credits') {
		message.channel.send(
			`** ${message.author.username}, your :credit_card: balance is ${
				games[message.author.id].credits
			}.**`
		);
	}
});
...
client.on('message', message => {
	...
	if (message.content.startsWith('$transfer')) {
		...
		if (!isNaN(money)) {
			message.channel.send(
				`**${message.author.username}**| يرجى وضع رقم صحيح `
			);
			return;
		}
		...
		if (!games[sender.id].credits) {
			message.channel.send(
				`**${sender.username}**| انت لا تملك دراهم كافية للتحويل`
			);
			return;
		}
		if (money > games[sender.id].credits) {
			...
			message.channel
				.send(`:money_with_wings: **_عملية تحويل_**:money_with_wings: 
...
client.on('message', message => {
	...
	if (message.content.startsWith('$info')) {
		...
		if (!args)
			return message.channel.send(
				`**${message.author.username}, يرجى كتابة المعلومات**`
			);
		if (args.length > 25)
			return message.channel.send(
				`**${message.author.username} يجب ان لا تكون المعلومات اكثر من 25 حرف**`
			);
		profile[message.author.id].info = args;
		message.channel.send(
			`**${message.author.username}**| تم تغير معلوماتك الى  $${args}>`
		);
	}
});
//لفل
...
client.on('message', async message => {
	...
	if (message.content.startsWith('$profile')) {
		...
		const w = ['./walls/p1.jpg'];
		if (!games[getvalueof.id])
			games[getvalueof.id] = {
				wins: 0,
				loses: 0,
				points: 0,
		...
		jimp.read(url, (err, ava) => {
			...
			ava.getBuffer(jimp.MIME_PNG, (err, buf) => {
				...
				message.channel.send({
					files: [{ attachment: canvas.toBuffer(), name: 'canvas.png' }]
				});
			});
		});
	}
